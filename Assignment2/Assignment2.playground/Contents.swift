...
The _ characters allow for map2 to be invoked with the rows and cols parameters being indicated by position not name.
...
/*
A function that accepts two Ints and returns something of type T
 */
/*:
...
struct Grid {
    ...
    init(_ rows: Int,
        ...
        self.rows = rows
        self.cols = cols
        self.cells = [[Cell]](repeating: [Cell](repeating: Cell(), count: cols), count: rows)
...
/*
 The word of is an external name for the Cell paramter and will be used when invoking this function
 */
/*:
...
/*
  The word cell is an internal name for the Cell paramter and will be used inside the body of the function.
 */
...
extension Grid {
    ...
    func neighbors(of cell: Cell) -> [Position] {
        return Grid.offsets.map {
            ...
            return Position(row: (rows + $0 + cell.position.row) % rows, col: (cols + $1 + cell.position.col) % cols)
        }
    }
}
...
/*
 To return an Int based on the values of the three Ints it is passed.
 */
/*:
...
/*
 This function is only to be used for returning Ints from objects of type Array of Array of Int.
 */
...
extension Grid {
    subscript (row: Int, col: Int) -> Cell? {
        get {
            ...
            guard (0<=row && row < self.rows && 0<=col && col<self.cols) else { return nil }
            return self.cells[row][col]
        }
        set {
            // ** Your Problem 14 `set` code goes here! replace the following line **
            guard (newValue != nil && 0<=row && row < self.rows && 0<=col && col<self.cols) else { return }
...
/*
 The reduce function in this extension returns the number of living neighbors a cell has.
 */
...
/*
The $1 refers to the Position tuple inside of the current Cell which we are evaluating
 */
/*:
 ## Problem 18:
...
extension Grid {
    func nextState(cell: Cell) -> CellState {
        // ** Problem 19 code goes here! Replace the following line **
        guard let cell = self[cell.position.row, cell.position.col] else { return .empty }
...
extension Grid {
    func next() -> Grid {
        ...
        map2(self.rows, self.cols) { (row, col)  in
            ...
            nextGrid[row,col]?.state = self.nextState(cell: self[row,col]!)
        }
        return nextGrid
    }
}
...
/*
  nextGrid represents the next state of all the cells 1 tick from the current state.
 */
/*:
 ## Problem 22:
